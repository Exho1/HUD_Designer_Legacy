...
function HD.OpenDesigner(firstime)
	...
	for i = 1, table.Count(HD.Tools) do
		...
		HD.ToolbarButtons[k]:SetTextColor(color_black)
		...
		HD.ToolbarButtons[k].Paint = function()
			...
			draw.RoundedBox(0, 2, 2, HD.ToolbarButtons[k]:GetWide()-4, HD.ToolbarButtons[k]:GetTall()-4, color_white)
		end
	end
				...
				if class == "draw.RoundedBox" then
					for id, data in pairs(objects) do
						if HD.LayerView then
							...
							draw.DrawText(HD.GetShapeLayer(id) or "", "Trebuchet24", data.x + 5, data.y, color_white)
						else
...
hook.Add("HUDShouldDraw", "HD_HideElements", function(name)
	if HD.DesignerOpen and shouldhide[name] then
		return false
	end
end)
